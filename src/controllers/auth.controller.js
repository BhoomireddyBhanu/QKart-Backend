...
const register = catchAsync(async (req, res) => {
  ...
  // console.log(newUser);
  var auth = await tokenService.generateAuthTokens(newUser);
  const response = {
    "user": newUser,
    "tokens": auth
  }
...
const login = catchAsync(async (req, res) => {
  ...
  let data = req.body;
  // console.log(data);
  var userData = await authService.loginUserWithEmailAndPassword(data.email, data.password);
  // console.log(userData)
  if(!userData){
    throw new ApiError(httpStatus.UNAUTHORIZED,"Incorrect email or password");
  }
  var auth = await tokenService.generateAuthTokens(userData);
  const response = {
    "user": userData,
    "tokens": auth
  }
