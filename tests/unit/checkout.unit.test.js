...
describe("Cart test", () => {
  ...
  describe("Checkout", () => {
    ...
    it("should update user balance and empty the cart on success", async () => {
      ...
      const previousBalance = userOneFinal.walletMoney;
      const res = await cartService.checkout(userOneFinal);
      ...
      expect(res.email).toEqual(userOneFinal.email);
      expect( userOneFinal.walletMoney).toBeLessThan(previousBalance);
      // expect(true).toEqual(false);
    });
  });
});
